...
class Client {
    constructor(web3, zsc, home, signers) {
        ...
        /**
         * 
         * @param {*} value token amount for withdraw
         * @param {*} setSimulateBlockime simulate block time synced with current time in the case of hardhat network
         * @returns 
         */
        this.withdraw = async (value, setSimulateBlockime) => {
            if (this.account.keypair === undefined)
                throw "Client's account is not yet registered!";
            const account = this.account;
            ...
            if(setSimulateBlockime) await setSimulateBlockime();
